...
pixel_t **alloc_px_mat(int size1, int size2)
{
	...
	for (int i = 0; i < size1; i++) {
		mat[i] = (pixel_t *)calloc(size2, sizeof(pixel_t));
		if (!mat[i]) {
			fprintf(stderr, "alloc failure\n");
			for (int j = 0; j < i; j++) {
				free(mat[j]);
...
image_t load_binary_pgm(FILE *in, image_t *img)
{
	for (int i = 0; i < img->height; i++) {
		for (int j = 0; j < img->width; j++) {
			...
			if (fread(&img->pixel_mat[i][j].r, size, 1, in) != 1) {
...
image_t load_binary_ppm(FILE *in, image_t *img)
{
	for (int i = 0; i < img->height; i++) {
		for (int j = 0; j < img->width; j++) {
			int size = sizeof(unsigned char);
			if (fread(&img->pixel_mat[i][j].r, size, 1, in) != 1 ||
				fread(&img->pixel_mat[i][j].g, size, 1, in) != 1 ||
				fread(&img->pixel_mat[i][j].b, size, 1, in) != 1) {
	...
	if (is_ascii) {
		...
		if (img->type[1] == '1' || img->type[1] == '2' ||
			...
			for (int i = 0; i < img->height; i++) {
				for (int j = 0; j < img->width; j++) {
					int size = sizeof(unsigned char);
					fwrite(&img->pixel_mat[i][j].r, size, 1, out);
				}
			}
		} else if (img->type[1] == '3' || img->type[1] == '6') {
			for (int i = 0; i < img->height; i++) {
				for (int j = 0; j < img->width; j++) {
					int size = sizeof(unsigned char);
					fwrite(&img->pixel_mat[i][j].r, size, 1, out);
					fwrite(&img->pixel_mat[i][j].g, size, 1, out);
					fwrite(&img->pixel_mat[i][j].b, size, 1, out);
				}
			}
		}
		fclose(out);
...
void rotate(image_t *img, selection_t *selection, int angle)
{
	...
	if (is_selection_all(img, selection)) {
		if (angle == 90 || angle == -270) {
			...
			for (int i = 0; i < img->height; i++) {
				for (int j = 0; j < img->width; j++) {
					new_mat[img->height - i - 1][img->width - j - 1] =
					img->pixel_mat[i][j];
				}
			}
		...
		for (int i = selection->y1; i < selection->y2; i++) {
			for (int j = selection->x1; j < selection->x2; j++) {
				if (angle == 90 || angle == -270) {
					new_mat[j - selection->x1][selection->y2 - i - 1] =
					img->pixel_mat[i][j];
				} else if (angle == 180 || angle == -180) {
					new_mat[selection->y2 - i - 1][selection->x2 - j - 1] =
					img->pixel_mat[i][j];
				} else if (angle == 270 || angle == -90) {
					new_mat[selection->x2 - j - 1][i - selection->y1] =
					img->pixel_mat[i][j];
				} else {
					new_mat[i - selection->y1][j - selection->x1] =
					img->pixel_mat[i][j];
				}
			}
		}
...
void crop(image_t *img, selection_t *selection)
{
	...
	for (int i = 0; i < new_height; i++) {
		for (int j = 0; j < new_width; j++) {
			new_mat[i][j] =
			img->pixel_mat[i + selection->y1][j + selection->x1];
		}
	}
...
void histogram(image_t *img, selection_t *selection, int mx_stars, int nr_bins)
{
	...
	for (int i = 0; i < nr_bins; i++) {
		int nr_stars = (new_freq[i] * mx_stars) / maxfreq;
		printf("%d\t|\t", nr_stars);
...
void handle_histogram(image_t *img, selection_t *selection, char *buffer)
{
	if (img->height == 0) {
		...
		if (sscanf(buffer, "%*s %d %d", &max_stars, &nr_of_bins) < 2) {
			...
			int trash1 = 0;
			if (sscanf(buffer, "%*s %*d %*d %d", &trash1) > 0) {
				printf("Invalid command\n");
			} else {
...
void equalize(image_t *img)
{
	...
	for (int i = 0; i < img->height; i++) {
		for (int j = 0; j < img->width; j++) {
			...
			int new_value =
			(int)round((double)cumulative[old_value] * 255 / total_pixels);
...
{
	pixel_t **new_mat =
	alloc_px_mat(selection->y2 - selection->y1, selection->x2 - selection->x1);
	if (!new_mat)
		return;
	...
	for (int i = selection->y1; i < selection->y2; i++) {
		for (int j = selection->x1; j < selection->x2; j++) {
			...
			if (!is_edge) {
				new_mat[i - selection->y1][j - selection->x1].r =
				clamp((int)round(sum_r / 16));
				new_mat[i - selection->y1][j - selection->x1].g =
				clamp((int)round(sum_g / 16));
				new_mat[i - selection->y1][j - selection->x1].b =
				clamp((int)round(sum_b / 16));
			} else {
				new_mat[i - selection->y1][j - selection->x1].r =
				img->pixel_mat[i][j].r;
				new_mat[i - selection->y1][j - selection->x1].g =
